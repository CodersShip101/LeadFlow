...
interface FilteredLead extends ScrapedPost {
...
async function scrapeWWR(): Promise<ScrapedPost[]> {
  try {
    ...
    console.error('WWR scrape error:', (e as Error).message)
    return []
  }
}

async function scrapeRemotive(): Promise<ScrapedPost[]> {
  try {
    ...
    console.error('Remotive scrape error:', (e as Error).message)
    return []
  }
}

async function aiFilter(post: ScrapedPost): Promise<FilteredLead[]> {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.error(msg)
      return [{ ...post, error: msg, budget_min: null, budget_max: null, project_type: null, skills_required: [], client_location: 'Remote', quality_score: 0, status: 'active' }]
    }
    ...
    console.error(`AI filter exception for "${post.title.substring(0, 50)}":`, (e as Error).message)
    return []
  }
}

async function insertLeads(leads: FilteredLead[]): Promise<number> {
  if (leads.length === 0) return 0
  const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL
  const serviceKey = process.env.SUPABASE_SERVICE_ROLE_KEY
  if (!supabaseUrl || !serviceKey) {
    console.error('Missing SUPABASE env vars for insert')
    return 0
  }
...
export async function scrapeAll(): Promise<ScrapeResult> {
  ...
  const results: ScrapeResult = { reddit: 0, wwr: 0, remotive: 0, accepted: 0, rejected: 0, inserted: 0, avgScore: '0', duration: 0, errors: [] }
  ...
  const batchSize = 1
  const allFiltered: FilteredLead[] = []
