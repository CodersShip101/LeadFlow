...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen">
      {/* Sidebar */}
      <aside className="hidden md:flex w-[220px] shrink-0 flex-col" style={{ background: 'var(--ink-2)', borderRight: '1px solid var(--border)' }}>
        <div className="flex items-center gap-2.5 px-4 h-14 shrink-0 border-b" style={{ borderColor: 'var(--border)' }}>
          ...
          <span className="text-sm font-semibold" style={{ color: 'var(--cream)' }}>LeadFlow</span>
        </div>
          ...
          {navItems.map(item => {
            ...
            return (
              <button key={item.label} onClick={() => router.push(item.href)} className={`sidebar-link ${active ? 'active' : ''}`}>
                ...
                {item.label === 'Feed' && newCount > 0 && <span className="ml-auto w-2 h-2 rounded-full" style={{ background: 'var(--amber)' }} />}
              </button>
            )
          })}
        </nav>
        <div className="shrink-0 px-3 py-3 border-t space-y-1" style={{ borderColor: 'var(--border)' }}>
          <button onClick={() => setDark(!dark)} className="sidebar-link text-xs">
            <i className={`ti ${dark ? 'ti-sun' : 'ti-moon'}`} /> {dark ? 'Light mode' : 'Dark mode'}
          </button>
          <button onClick={handleLogout} className="sidebar-link text-xs" style={{ color: 'var(--red-score)' }}>
      ...
      <div className="flex-1 flex flex-col min-w-0 max-w-full">
        {profile && (!profile.skills || profile.skills.length === 0 || !profile.hourly_rate) && (
          <div className="flex items-center gap-2.5 px-4 md:px-8 py-2.5 text-xs font-medium animate-fade-in" style={{ background: 'var(--amber-pale)', borderBottom: '1px solid var(--amber-dim)' }}>
            <i className="ti ti-alert-triangle" style={{ color: 'var(--amber)' }} />
            <span style={{ color: 'var(--amber)' }}>Complete your profile to get matched leads.</span>
            <button onClick={() => router.push('/dashboard/profile')} className="underline font-semibold ml-auto" style={{ color: 'var(--amber)' }}>Add skills →</button>
          </div>
        )}
        {children}
      </div>
      ...
      {/* Bottom nav */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-40" style={{ background: 'var(--ink-2)', borderTop: '1px solid var(--border)', paddingBottom: 'env(safe-area-inset-bottom, 0px)' }}>
          ...
          {bottomItems.map(item => {
            ...
            return (
              <button key={item.label} onClick={() => router.push(item.href)}
                ...
                style={{ color: active ? 'var(--amber)' : 'var(--slate)' }}>
