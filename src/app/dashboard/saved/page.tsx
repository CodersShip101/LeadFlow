...
export default function SavedPage() {
  ...
  return (
    <div className="flex-1 px-4 md:px-8 pt-6 pb-20 md:pb-8">
      <h1 className="text-lg font-bold" style={{ color: 'var(--base-900)' }}>Saved Leads</h1>
      <p className="text-xs mt-1 mb-4" style={{ color: 'var(--base-500)' }}>Bookmarked for later review</p>
      {leads.length === 0 ? (
        <div className="text-center py-16">
          <i className="ti ti-bookmark text-2xl" style={{ color: 'var(--base-300)' }} />
          <p className="text-sm mt-2" style={{ color: 'var(--base-500)' }}>No saved leads yet</p>
          ...
          {leads.map(lead => (
            <div key={lead.id} onClick={() => router.push(`/dashboard/lead/${lead.id}`)}
              ...
              <div className="flex-1 min-w-0">
                ...
                <h3 className="text-sm font-semibold line-clamp-1" style={{ color: 'var(--base-900)' }}>{lead.title}</h3>
