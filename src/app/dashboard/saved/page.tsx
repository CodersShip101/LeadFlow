...
export default function SavedPage() {
  ...
  return (
    <div className="flex-1 px-4 md:px-8 pt-6 pb-20 md:pb-8">
      <h1 className="text-lg font-bold" style={{ color: 'var(--cream)' }}>Saved Leads</h1>
      <p className="text-xs mt-1 mb-4" style={{ color: 'var(--slate)' }}>Bookmarked for later review</p>
      {leads.length === 0 ? (
        <div className="text-center py-16">
          <i className="ti ti-bookmark text-2xl" style={{ color: 'var(--slate-3)' }} />
          <p className="text-sm mt-2" style={{ color: 'var(--slate)' }}>No saved leads yet</p>
          <button onClick={() => router.push('/dashboard')} className="btn-p text-sm mt-4">Browse leads</button>
        </div>
      ) : (
        <div className="space-y-2">
          {leads.map(lead => (
            <div key={lead.id} onClick={() => router.push(`/dashboard/lead/${lead.id}`)}
              className="lead-card px-4 py-3 cursor-pointer flex items-center gap-3">
              <div className="flex-1 min-w-0">
                ...
                <h3 className="text-sm font-semibold line-clamp-1" style={{ color: 'var(--cream)' }}>{lead.title}</h3>
              ...
              <button onClick={e => { e.stopPropagation(); unsave(lead.id) }}
                className="btn-g text-xs min-h-[36px] min-w-[36px] flex items-center justify-center shrink-0"
                style={{ color: 'var(--red-score)' }}>
